...
async def main():
    if len(sys.argv) < 2:
        console.print("[yellow]Usage:[/yellow] uv run python scripts/check_sni.py <domain>")
        console.print("[dim]Example: uv run python scripts/check_sni.py api.fmhy.net[/dim]")
        sys.exit(1)
    ...
    if 'Table' in globals():
        ...
        for ip, (success, reason) in zip(ip_list, results):
        ...
        for ip, (success, reason) in zip(ip_list, results):
            ...
            reset = "\033[0m"
            print(f"{ip:<15} ({proxies[ip]:<18}) -> {color}{status:<5}{reset} : {reason}")
