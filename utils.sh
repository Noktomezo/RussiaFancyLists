...
merge_hosts() {
  ...
  awk -v ip_file="$sni_proxy_ip_file" '
...

    {
      ...
      # 1. Если строка начинается с 0.0.0.0 или 127.0.0.1 — СКИПАЕМ (это мусор)
      if ($1 == "0.0.0.0" || $1 == "127.0.0.1") next

      # 2. Определяем домен
      # Если $1 это любой другой IP (например из твоих старых дампов), берем $2
      # Если $1 это уже домен, берем его
      dom = ($1 ~ /^[0-9.]+$/) ? $2 : $1
      if (!dom) next
