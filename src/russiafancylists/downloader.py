...
async def download_file(
    ...
    for attempt in range(1, retries + 1):
        try:
            progress.update(task_id, description=f"[cyan]Downloading {description}...[/cyan]")
            ...
            if attempt == retries:
                ...
                raise RuntimeError(f"Failed to download {description} from {url} after {retries} attempts: {e}")
            else:
                progress.update(task_id, description=f"[yellow]⚠ {description} retry {attempt}/{retries}...[/yellow]")
...
async def run_downloads(progress: Progress):
    ...
    async with httpx.AsyncClient(limits=limits, verify=False) as client:
        tasks = []
        for name, (url, path, desc) in DOWNLOADS.items():
