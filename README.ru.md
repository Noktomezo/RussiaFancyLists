...
> [!CAUTION]
> ⚠️ **Нестабильность Combined-списка**: Файл `combined.lst` смешивает IP-адреса прокси от разных провайдеров. Это может привести к нестабильной работе и крайне усложняет диагностику, так как невозможно понять, на стороне какого именно провайдера возникли проблемы. Настоятельно рекомендуется использовать список конкретного провайдера (например, `malw.lst`, `mafioznik.lst` или `geohide.lst`).
